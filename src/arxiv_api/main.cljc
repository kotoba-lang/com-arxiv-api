(ns arxiv_api.main
  "Kotodama WASM entrypoint for the Arxiv_api clean-room actor (L4) — Clojure port.
  ...
  facts (`arxiv_api.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "arxiv_api")
(def tier "L4")
...
(def entity-specs
  [{:entity "Work"       :plural "works"       :id-prefix "arxivapi_wor"
    :fields [:doi :title :type :publishedAt]   :required [:doi :title]
    :coerce {}                       :refs {}}
   {:entity "Author"     :plural "authors"     :id-prefix "arxivapi_aut"
    :fields [:orcid :givenName :familyName]    :required [:orcid :givenName]
    :coerce {}                       :refs {}}
   {:entity "Citation"   :plural "citations"   :id-prefix "arxivapi_cit"
    :fields [:workId :citedDoi]                :required [:citedDoi]
    :coerce {}                       :refs {:workId "Work"}}
   {:entity "Standard"   :plural "standards"   :id-prefix "arxivapi_sta"
    :fields [:designation :title :organization :status] :required [:designation :title]
    :coerce {}                       :refs {}}
   {:entity "Record"     :plural "records"     :id-prefix "arxivapi_rec"
    :fields [:accession :database :title]      :required [:accession :database]
    :coerce {}                       :refs {}}
   {:entity "Subject"    :plural "subjects"    :id-prefix "arxivapi_sub"
    :fields [:workId :term :scheme]            :required [:term :scheme]
    :coerce {}                       :refs {:workId "Work"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"arxiv_api.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "arxiv_api-compat" :tier tier :entities entities} 200])
...
(defn start! [] :arxiv_api-compat/ready)
